...
// ── DYNAMISCHE TURN CREDENTIALS HOLEN ──────────────────────────────────
async function fetchIceServers() {
    try {
        // Holt die zeitlich begrenzten Zugangsdaten vom Node.js-Backend über Nginx
        ...
        console.error("Fehler beim Laden der TURN-Zugangsdaten, Fallback auf STUN:", error);
        // Fallback: Wenn das Node-Backend offline ist, wird zumindest STUN versucht
        return [{
            urls: 'stun:nicohartmann.dev:3478'
        }];
    }
...
// Hilfsfunktion zur Erstellung der Peer-Konfiguration
...
document.getElementById('md-input').addEventListener('change', function(e) {
    ...
    reader.onload = (event) => {
        try {
            ...
            if (rawData.length === 0) throw "Keine Fragen gefunden.";
            ...
            const parsed = rawData.map((block, idx) => {
                ...
                if (answers.length < 2) throw `Frage ${idx+1}: Mindestens 2 Antworten nötig.`;
                if (correctCount !== 1) throw `Frage ${idx+1}: Genau eine richtige Antwort mit [x] markieren.`;
                return {
                    question,
                    answers
                };
            ...
            errorEl.innerText = "⚠️ FEHLER: " + err;
...
async function setupLecturer() {
    ...
    setStatus('connecting', 'Lade Sicherheits-Token…');

    // Erst die dynamischen Server-Daten holen
    ...
    peer.on('error', err => {
        ...
        setStatus('error', 'Verbindungsfehler – Seite neu laden');
    });
...
function generateQR(id) {
    ...
    // Bereinigt alten QR-Code vor dem Neu-Generieren
...
async function joinRoom() {
    ...
    if (!roomId || !name) {
        errEl.innerText = '⚠️ Bitte Raum-Code und Nickname eingeben.';
    ...
    peer.on('error', err => {
        ...
        if (err.type === 'peer-unavailable') {
            setStatus('error', 'Raum nicht gefunden');
            errEl.innerText = '⚠️ Raum nicht gefunden. Code prüfen und erneut versuchen.';
            errEl.classList.remove('hidden');
        } else {
            setStatus('error', 'Verbindungsfehler');
            errEl.innerText = '⚠️ Verbindungsfehler: ' + (err.message || err.type);
            errEl.classList.remove('hidden');
        }
    });
    ...
    peer.on('open', (id) => {
        ...
        const timeout = setTimeout(() => {
            setStatus('error', 'Zeitüberschreitung');
            errEl.innerText = '⚠️ Keine Antwort vom Host. Raum-Code prüfen.';
            errEl.classList.remove('hidden');
        }, 8000);
        ...
        conn.on('close', () => {
            setStatus('error', 'Verbindung getrennt');
        });
    });
}
...
function broadcastQuestion() {
    ...
    document.getElementById('host-q-count').innerText = `Frage ${currentQuestion + 1} von ${quizData.length}`;
...
function renderQuestion(q, num, total) {
    ...
    showView('quiz');
    document.getElementById('student-q-count').innerText = `Frage ${num} von ${total}`;
        ...
        if (answeredAnswer) {
            ...
            resultBanner.innerHTML = `
                        ...
                        <div class="result-text">
                            <div class="result-label">Deine Antwort</div>
            ...
            resultBanner.innerHTML = `
                        ...
                        <div class="result-text">
                            <div class="result-label">Deine Antwort</div>
                            <div class="result-answer-text">Keine Antwort</div>
                        </div>
                        <div class="result-points">
...
// ── Antwort & Punkte im handleAnswer tracken ──────────────────────────
